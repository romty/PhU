...
#This Simulator is used for paper "DeepInSAR: A Deep Learning Framework for SAR Interferometric Phase Restoration and Coherence Estimation". 
#Please check out the branch 'DeepInSAR-v1' to get the same version which is used in paper's experiments.
...
import scipy.io
def example_1(name):
  sim = IfgSim(width=256, height=256, rayleigh_scale=0.9)
  #sim.add_n_buildings(width_range=[10,100], height_range=[1,40], depth_factor=0.35, nps=25)
  sim.add_n_gauss_bubbles(sigma_range=[10,150], amp_range=[-4.5,4.5], nps=110)
  sim.add_n_amp_stripes(thickness=9, nps=5)
  sim.add_n_amp_stripes(thickness=3, nps=50)
  ...
  scipy.io.savemat('PU/wraped_'+name+'.mat',{'wrap':sim.ifg})
    #scipy.io.savemat('/content/drive/Colab Notebooks/sim_data/for_noisydata/wrapednoisy_'+name+'.mat',{'wrap':np.angle(sim.ifg_noisy)})
